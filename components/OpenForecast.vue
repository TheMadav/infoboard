<template>
	<div
		...
		<div
			...
			:key="index"
			class="forecastWrapper col mx-2 mb-2">
			<div class="forecast m-auto">
				<p>{{ moment.unix(weather.dt).format('HH:MM') }} Uhr</p>
				<div class="forecastIcon"><img
					:src="'/images/'+weather.weather[0].main.toLowerCase()+'.svg'"
					:alt="weather.weather[0].summary"></div>
				<div>{{ roundValue(weather.main.temp) }}&deg;{{ units }}  <br/> <small>{{ weather.weather[0].description }}</small></div>
